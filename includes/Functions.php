		...
		try {
			...
			if ( isset( $_POST['params'] ) ) {
				...
				if ( ! empty( $valSearch ) ) {
					...
					$sqlRepareAll = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs WHERE $whereQuery" );

					$sqlRepare = $wpdb->prepare(
						"SELECT * FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs WHERE $whereQuery ORDER BY $sortField $sortVal LIMIT %d OFFSET %d",
						$limit,
						$offset
					);
				} else {
					$sqlRepareAll = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs WHERE $statusWhere" );

					$sqlRepare = $wpdb->prepare(
						"SELECT * FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs WHERE $statusWhere ORDER BY $sortField $sortVal LIMIT %d OFFSET %d",
						$limit,
						$offset
					);
				}
