...
class Schedule {
	...
	private function doHooks() {
		...
		add_action( YAY_SMTP_SENDINBLUE_PREFIX . '_delete_email_log_schedule_hook', array( $this, 'delete_email_log_schedule' ) );

		if ( ! wp_next_scheduled( YAY_SMTP_SENDINBLUE_PREFIX . '_delete_email_log_schedule_hook' ) ) {
			$timeSch = YAY_SMTP_SENDINBLUE_PREFIX . '_specific_delete_time';
			wp_schedule_event( time(), $timeSch, YAY_SMTP_SENDINBLUE_PREFIX . '_delete_email_log_schedule_hook' );
		}
	}
	...
	public function delete_email_log_schedule() {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . $wpdb->prefix . 'yay_smtp_sendinblue_email_logs' ) );
	}
